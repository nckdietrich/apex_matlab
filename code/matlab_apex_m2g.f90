! Running apex_mall - Converts  to apex coords to geodetic coords
! Author: Nick Dietrich
! Version 4/1/22

program matlab_apex_m2g
...
!! Declaring 
character(len=128) :: file_in = 'apex_pos.nc'
...
real :: date, altmax, hr
real, allocatable :: apex_loc(:,:), aplons(:), aplats(:), alts(:)
! Apex output variables
real :: gdlat, gdlon
real, allocatable :: gd_loc(:,:), ier_list(:)

!! Running
print *, 'Converting Modified Apex Coords to Geodetic Coords'
! Read in information
! Get vector length
...
call check( nf90_get_var(in_ncid, varid, values=apex_loc) )
! date
...
allocate(aplons(nloc))
allocate(aplats(nloc))
allocate(alts(nloc))
aplons = apex_loc(:,1)
aplats = apex_loc(:,2)
alts = apex_loc(:,3)
...
do loc_ind = 1, nloc
    call apex_m2g( aplats(loc_ind), aplons(loc_ind), alts(loc_ind),&
        hr, gdlat, gdlon)
